...
Function New-BGinfo {
	Param(  [Parameter(Mandatory)]
			...
			[Parameter()]
			[string] $OutFile= "$PSScriptRoot\" + ( ( get-date ).TimeOfDay.TotalSeconds ) + "BGInfo.bmp",
	...
	Begin {
		...
		#     }
		# }
		$wpath = "C:\Users\rerdmann\Pictures\Surface_Owl_QHD.jpg"
		...
		Try {
			...
			Write-Warning -Message "Failed 2 Failed to $($_.Exception.Message)"
			break
		}
	}
	Process {
		...
		# Try {
			...
			# 	)
			# 	$sz2 = [system.windows.forms.textrenderer]::MeasureText($arttt, $font2)
			# 	$rect2 = New-Object System.Drawing.RectangleF (0,($i*$FontSize*1.5 + $sz2.Height),$SR.Width,$SR.Height) -ErrorAction Stop
			# 	$image.DrawString($arttt, $font2, $brush2, $rect2, $sFormat)
			# }
			for ($i ; $i -le $artext.Count ; $i++) {
			    if ($i -eq 0) {
			        ...
			        $font2 = New-Object System.Drawing.Font($FontName,$FS2,[System.Drawing.FontStyle]::Regular)
			        $Brush2 = New-Object Drawing.SolidBrush (
			            [System.Drawing.Color]::FromArgb($FC2[0],$FC2[1],$FC2[2])
			        )
		...
		# } Catch {
		# 	Write-Warning -Message "Failed 3 to $($_.Exception.Message)"
		# 	break
		# }
	}
	End {
		Try {
			...
			Write-Warning -Message "Failed to 5 $($_.Exception.Message)"
			break
		}
	}
...
"@ -ErrorAction Stop
		...
		# } Catch TYPE_ALREADY_EXISTS
			...
			Write-Warning -Message "Failed because 7 $($_.Exception.Message)"
		}
